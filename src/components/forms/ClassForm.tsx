...
export default function ClassForm({ type, data, setOpen, onSuccess }: Props) {
  const {
    register,
    handleSubmit,
    reset,
    ...
    resolver: zodResolver(classSchema) as any, // ✅ cast to any to fix unknown <> number issue
    defaultValues: {
      id: data?.id,
      name: data?.name || "",
  ...
  const onSubmit: SubmitHandler<ClassSchema> = async (formData) => {
    try {
      let result;
      if (type === "create") {
        result = await createClass({ success: false, error: false }, formData);
      } else {
        result = await updateClass({ success: false, error: false }, formData);
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        <label htmlFor="gradeId" className="block font-medium">
          Grade
        </label>
        <select
          id="gradeId"
      ...
      <div>
        <label htmlFor="supervisorId" className="block font-medium">
          Supervisor ID
        </label>
        <input
          id="supervisorId"
